...
try:
    ...
    RSS_AVAILABLE = False
    logger.warning(f"RSS采集器导入失败: {e}，将使用模拟数据")
...
class HotTopicAgent:
    ...
    def collect(
        ...
        参数:
            ...
            limit: 采集上限
            use_rss: 是否使用RSS采集
            use_api: 是否使用API采集（需要配置API密钥）
            use_sample: 是否使用模拟数据（仅用于测试）
        ...
        category_map = {
            ...
            "教育": "教育", "EDUCATION": "教育",
            "健康": "健康", "HEALTH": "健康",
            "国际": "国际", "INTERNATIONAL": "国际",
        }
        
        if params["categories"]:
            rss_categories = [category_map.get(c, c) for c in params["categories"]]

        # 采集策略
        events = []

        # 策略1: RSS采集（推荐）
        if use_rss and RSS_AVAILABLE:
            try:
                rss_collector = RSSNewsCollector(
                    cache_dir=f"{self.output_dir}/cache/rss"
                )
                ...
                self.logger.info(f"RSS采集完成，获取 {len(events)} 条事件")
                
            except Exception as e:
                self.logger.error(f"RSS采集失败: {e}")
                events = []

        # 策略2: API采集（如果配置了API密钥）
        if use_api and len(self.api_keys.get("openai_api_key", "")) > 10:
            self.logger.info("使用API采集（需要OpenAI API密钥）")
            # TODO: 实现API采集逻辑
            self.logger.warning("API采集功能待实现")
        
        # 策略3: 模拟数据（用于测试或演示）
        if use_sample or (len(events) == 0 and not use_rss):
            self.logger.info("使用模拟数据进行测试...")
            sample_events = self._generate_sample_events(params["limit"])
            events.extend(sample_events)
            self.logger.info(f"模拟数据生成 {len(sample_events)} 条事件")

        # 过滤关键词
        if params["keywords"]:
            filtered_events = []
            for event in events:
                title_summary = event.title + " " + event.summary
                if any(kw in title_summary for kw in params["keywords"]):
